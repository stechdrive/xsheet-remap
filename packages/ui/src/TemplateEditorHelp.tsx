...
const templateHelpChapters: TemplateHelpChapter[] = [
  {
    ...
    steps: [
      ...
      { title: '開始方法を選ぶ', description: '最初の画面で「A3標準」「参照画像」「デジタル」「既存JSON」から、目的に合う入口を1つ選びます。' },
      { title: '作成条件を確認する', description: '紙は表示される実ピクセル数と参照画像のPPIを確認します。デジタルは実運用のFPSと必要な共有セル列数を先に決め、「作成」を押します。' },
      { title: 'IDと名前を整える', description: '右の「テンプレート」でIDと表示名を確認します。IDは他のテンプレートと重複しない英数字中心の名前、表示名は作業者が選べるわかりやすい名前にします。' },
      { title: '紙の参照画像を合わせる', description: '紙の場合は「参照画像」で画像、ピクセル数、PPI、配置を確認します。テンプレートと画像のPPIが異なるときは先に原画像側を見直します。デジタルではこの手順を飛ばします。' },
      { title: '必要な領域を追加する', description: '上部の「＋ 領域」から必要なものだけを追加します。左の領域一覧で選択、表示、位置固定、複製、重なり順、削除を管理できます。' },
      { title: 'キャンバス上で位置を決める', description: '領域をクリックし、赤い辺またはハンドルをドラッグして大きさを合わせます。右の「選択領域」にあるX・Y・W・Hを数値入力すると、1px単位で仕上げられます。' },
      { title: '領域の中身を設定する', description: '「全領域」で領域名、情報の割当、文字サイズ、開始F、行数、列数を設定します。入力表は「選択領域」で編集方法、メモ対象、文字と内余白を設定します。' },
      { title: '表示ルールを整える', description: '「表示」で紙の下端セル列名、ACTION・SOUND・CELL・CAMERAの見出し文字、各列名の表示／非表示を確認します。SOUND・CAMERAは新規プロジェクトで使う初期列名も編集できます。' },
      { title: '紙の補正基準枠を決める', description: '紙の「テンプレート」で補正基準枠を編集対象にし、グリッド外接、A3標準枠、手入力のどれかで決めます。この枠がEditorとCorrectorの四隅補正の基準になります。' },
      { title: '工程と登録先を確認する', description: '「工程編集」を開き、CSP上の上下順、工程名、ファイル名接尾辞を整えます。一番下の工程が素材を直接登録したときのデフォルト登録先です。' },
      { title: '確認・保存・実機確認', description: '右の「確認」でエラーを解消し、「確認して保存」でJSONを保存します。EditorまたはRemapでそのJSONを読み込み、クリック範囲、入力、ページ、紙補正をテストしたら完成です。' },
    ],
    sections: [
      {
        title: '完成判定のチェックリスト',
    ...
    sections: [
      {
        ...
        items: [
          ...
          { term: '上部ツールバー', description: '未保存状態、開始画面へ戻る、新規作成、JSON読込・保存、完成前確認、領域追加、工程編集、倍率をまとめています。' },
          ...
          { term: '右の詳細ドック', description: '「テンプレート」「選択領域」「表示」「参照画像」「全領域」「確認」「JSON」を切り替えます。参照画像は紙テンプレートにだけ表示されます。' },
    ...
    sections: [
      {
        ...
        items: [
          ...
          { term: '数値で仕上げ', description: '右の「選択領域」にあるX・Y・W・Hは実ピクセル単位です。元画像の罫線位置がわかる場合は数値入力が確実です。' },
        ],
      },
      {
        title: '表示移動と倍率',
    ...
    sections: [
      {
        title: '情報欄',
        items: [
          { term: '追加と割当', description: '「＋ 領域」の情報欄は、まだ使われていない作品名・話数・シーン・カット・尺・作業者名・ページ数などを順に割り当てます。「全領域」で別項目へ変更できます。' },
          { term: '文字レイアウト', description: '選択後の「選択領域」で文字サイズ、最小文字、行間、内余白を設定します。横・縦の欄外表示も選べます。' },
          { term: 'カット番号プレフィックス', description: '「テンプレート」で保存名に付ける接頭辞を指定します。「数字カットだけ」をオンにするとOPなどの文字カットには付加しません。' },
        ],
      },
      {
        title: '入力表',
    ...
    sections: [
      {
        ...
        items: [
          ...
          { term: 'SOUND', description: '台詞やSEなどの音響指示を、開始から終了までの区間として記入する欄です。「表示」で初期列名と列名を表示するかを決め、「全領域」で初期列数を変更します。' },
    ...
    sections: [
      {
        ...
        items: [
          ...
          { term: '行数と列数', description: '「選択領域」の補助罫線設定で、1〜64列と必要な行数を指定します。' },
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        title: '補正基準枠',
        items: [
          { term: '編集対象にする', description: '「テンプレート」の補正基準枠にあるボタンを押すと、キャンバス上の枠をドラッグまたはX・Y・W・Hで編集できます。' },
    ...
    sections: [
      {
        ...
        items: [
          ...
          { term: '選択領域', description: 'X・Y・W・H、文字レイアウト、入力表の欄設定、補助罫線の行列と線を、領域の種類に応じて表示します。' },
        ],
      },
      {
        title: '全領域・確認・JSON',
        items: [
          { term: '全領域', description: '全領域を表で一度に確認します。行のクリックでキャンバスの選択と連動し、領域名、情報割当、文字、X・Y・W・H、開始F、行数、列数を直接編集できます。' },
          { term: '確認', description: 'ID、名前、入力領域、ページ内配置、参照画像PPI、補正基準枠を保存前に検査します。エラーは保存前に修正し、注意は実機で確認します。' },
  ...
  return (
    <div className="appHelpBackdrop" role="dialog" aria-modal="true" aria-label="xsheet-templateの使い方">
      <section className="appHelpDialog templateEditorHelpDialog">
        ...
        <footer>
          <p>右の「確認」で問題を解消し、最後に「確認して保存」でJSONを保存してください。</p>
        </footer>
      </section>
    </div>
  )
