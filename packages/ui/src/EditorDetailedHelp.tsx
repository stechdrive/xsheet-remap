    ...
    sections: [
      {
        ...
        items: [
          ...
          { term: '録音時のVAD動作', description: '各トラックの録音VADを「VADなし」「候補」「仮SOUND」から選びます。仮SOUNDでは新しく録音または読み込んだ音声だけを解析して仮ラベルを自動作成します。複雑な掛け合いでは候補にして、音声編集後に必要な区間をまとめるのが便利です。' },
          ...
          { term: '保存範囲', description: '元音声はカット単位の共有資産、編集結果は資産を参照するクリップとしてXSRへ保存されます。同じ音声を分割してもXSR内ではハッシュBlobが重複しません。VAD、音声アンカー、SOUNDリンクはシート改訂ごとに保持し、旧形式の音声プロジェクトは読込時に自動移行します。' },
