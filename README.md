...
- セル列をAfter Effects Keyframe Dataまたはマッピング付きJSXとして渡す
...
確認用にはPNG/JPG/PSD、CSP連携用にはXDTSまたはCSP自動登録出力を使います。After Effectsへは、列見出しからKeyframe Dataをコピーするか、複数列をマッピングできるJSXを書き出します。Windowsデスクトップ版では、起動中のAfter Effectsが1つだけの場合に同じリマップ内容を直接送信できます。

CSP自動登録出力では素材画像そのものをコピーせず、登録ファイルからカットフォルダ内の既存素材を参照します。
