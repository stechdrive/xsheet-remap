...
describe('desktop e2e scenarios', () => {
  it('builds the full default A3 project and validates normalized XDTS output', () => {
    ...
    expect(scenario.project.assetRoot?.path).toBe('C:\\e2e\\assets')
    expect(scenario.project.assets.every(asset => asset.source.kind === 'root-relative')).toBe(true)
    ...
    expect(scenario.normalizedProject.assets.find(asset => asset.originalFileName === 'A1.png')?.source).toEqual({ kind: 'root-relative', relativePath: 'A_01.png' })
