...
describe('dialogue audio project extension', () => {
  ...
  it('stores shared audio assets and revision-specific candidate links per cut', () => {
    ...
    state.tracks[0] = {
      ...state.tracks[0],
      name: '主人公',
      clips: [{ clipId: 'clip-1', assetId: 'asset-1', timelineStartFrame: 3, sourceOffsetFrames: 0, durationFrames: 48 }],
      speechCandidates: [{ candidateId: 'candidate-1', frameStart: 3, frameEnd: 16, status: 'pending' }],
    }
    const linked = linkDialogueAudioCandidate(state, 'dialogue-1', 'candidate-1', 'cue-1', 'revision-1')
