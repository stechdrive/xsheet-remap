    ...
    sections: [
      {
        ...
        items: [
          ...
          { term: 'VADとSOUNDは別レイヤー', description: '波形上の緑のVAD区間は発話検出の根拠で、トラック上段のSOUNDラベルとは別データです。CtrlまたはShiftで複数のVAD区間を選び「候補をまとめてSOUND作成」を使うと、途中に間があってもアフレコ目安を1ブロックにできます。シートまたは音声タイムラインでSOUNDを伸縮しても波形は変わりません。' },
          { term: '録音時のVAD動作', description: '各トラックの録音VADを「VADなし」「候補」「セリフ区間」から選びます。セリフ区間では新しく録音または読み込んだ音声だけを解析し、タイムシートへ割り付ける前の区間を自動作成します。複雑な掛け合いでは候補にして、音声編集後に必要な区間をまとめるのが便利です。' },
          { term: 'Silero VADで発話を検出', description: 'KomaSyncと同じSilero VADモデルと区間補正で音声を解析します。「環境」を静か／通常／騒がしいから選び、検出感度と途切れにくさを調整して「セリフ区間を再検出」します。解析中とSilero VAD使用中は状態表示で確認できます。モデルを読み込めなかった場合だけ「簡易検出」と明示して切り替わります。' },
          { term: '再検出と同期', description: '検出設定を調整して再検出しても、リンク済み・無視済みの処理結果は保持します。無音挿入やリップル削除で後続音声を動かした場合はリンク済みSOUNDも同じフレーム数だけ移動します。リンク済み音声を無音化した場合や区間が一致しない場合はSOUNDを勝手に削除せず「要確認」として残します。' },
          { term: '音声アンカーと追従', description: 'SOUNDは複数の音声アンカーと前後余白を保持します。シート側でラベルを長くしても音声は動かず、その後にリンク音声を移動すると余白を保ったままSOUNDだけが追従します。音声を分割してもアンカーは子クリップへ残り、リンク音声がすべてなくなった場合もSOUNDを削除せず要確認として残します。' },
          { term: '保存範囲', description: '元音声はカット単位の共有資産、編集結果は資産を参照するクリップとしてXSRへ保存されます。同じ音声を分割してもXSR内ではハッシュBlobが重複しません。VAD、セリフ区間、音響指示への割付はシート改訂ごとに保持し、旧形式の音声プロジェクトは読込時に自動移行します。' },
