...
const editorHelpChapters: HelpChapter[] = [
  {
    ...
    number: '01',
    title: '画面の見方',
    summary: 'メニュー、ツールバー、シート、左右のペインの役割を把握します。',
    sections: [
      {
        title: '上部の操作',
        items: [
          ...
          { term: '紙シート', description: '紙タイムシート画像の読込、四隅補正、不透明度、レベル補正、デスクトップ版での文字認識を操作します。' },
          { term: '表示・履歴・ヘルプ', description: 'ページ表示方式と描画レイヤーの切替、元に戻す／やり直し、クイックガイドとこの詳細ヘルプを開きます。' },
        ],
      },
      {
        title: 'シート入力ワークスペース',
        items: [
          { term: '左：CSPレイヤー構成', description: '登録工程、セル、BG／BOOK、撮影指示、メモの構成とCSP上の重ね順を管理します。行やカードを選択し、最下部に固定された一括リネーム、追加、削除の各操作を使います。' },
          { term: '中央：タイムシート', description: 'ACTION、CELL、SOUND、CAMERAの各欄へ入力します。上のツールバーでテンプレート、兼用カット、登録先、ページ、倍率、注釈を操作します。' },
          { term: '右：画像素材', description: 'カットフォルダを参照し、画像の登録、選択、プレビュー、並べ替え、シートや登録セルへの割当を行います。' },
          { term: '右端：シート履歴', description: '初稿と修正シートを切り替えます。名前変更、保護、前シートの下敷き表示、追加、削除ができます。' },
    ...
    sections: [
      {
        ...
        items: [
          ...
          { term: 'プロジェクトを開く', description: '.xsr、.xsr.json、旧来のJSONプロジェクトを開きます。別PCで素材パスが見つからない場合は、画像素材のカットフォルダを登録し直します。' },
    ...
    sections: [
      {
        ...
        items: [
          ...
          { term: 'プリロール', description: 'シートツールバーの「前置き」で、本編前に表示する固定のダミーコマを切り替えます。本編のカット尺そのものには含まれません。' },
        ],
      },
      {
        title: '兼用カットと修正シート',
        items: [
          { term: '兼用カット', description: '「兼用」の＋でカットを追加し、一覧から切り替えます。登録セルと素材を共有しながら、タイミング、メモ、紙シート画像、修正履歴はカットごとに管理されます。' },
          { term: '兼用番号の表示', description: '「兼用カット番号」をオンにすると、同じグループのほかのカット番号をシートと画像出力へ表示します。最後に残った1カットは削除できません。' },
          { term: '修正シートを追加', description: '右端の＋から、現在のシートを引き継ぐか、空のシートを追加します。名前を付け、必要なら一つ前のシートを下敷きとして表示します。' },
          { term: '履歴の管理', description: 'シートの右クリックメニューから名前変更、編集保護、下敷き表示の切替、削除を行います。保護中のシートは意図しない編集を防ぎます。' },
        ],
      },
    ],
  },
    ...
    sections: [
      {
        ...
        items: [
          ...
          { term: 'XDTS連携', description: 'XDTS読込時にSOUND指示を取り込むか選択できます。書き出し時はタイミング書き出し設定で対象を確認します。' },
        ],
      },
    ],
  },
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          { term: '表示切替', description: '上部の表示メニューにある「注釈」で、内容を削除せず一時的に非表示にできます。' },
        ],
      },
    ],
  },
    ...
    sections: [
      {
        title: '画像の読込とページ割当',
        items: [
          { term: '紙シートを読み込む', description: '上部の「読込」から画像を選びます。複数画像はファイル名順にページへ割り当てられ、必要に応じてページ数と尺が更新されます。' },
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          { term: '登録先工程', description: 'シートツールバーの「登録先」で、新規入力キーや直接ドロップした素材を登録する工程を選びます。テンプレート側の工程設定では工程名、接尾辞、CSP上の上下順を管理します。' },
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          { term: '表示テンプレート', description: 'シートツールバーのテンプレートメニューで、プロジェクトに適用する表示様式を選びます。テンプレート変更は欄の位置、ページ寸法、印刷表現に影響します。' },
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          { term: 'XDTS', description: 'タイミングを対応ソフトへ渡す交換形式です。対象欄とSOUND／CAMERA指示などの出力設定を確認して保存します。' },
          { term: 'CSP自動登録データ', description: 'xsheet-importer用のcsp-import.xci、XDTS、素材参照などをまとめます。csp-import.xciはCLIP STUDIO PAINTへ直接読み込まず、同梱のxsheet-importerで選択します。' },
      ...
      },
      {
        title: 'デスクトップ版とPWA版',
        items: [
          { term: 'デスクトップ版', description: '保存先を選んで直接書き込み、素材のネイティブパス、OCR、CSP自動登録用の実ファイル参照を利用できます。' },
          { term: 'PWA版', description: 'プロジェクトや出力物はブラウザのダウンロードとして保存されます。複数ファイルの出力はZIPになる場合があります。OCRとネイティブパス前提の処理は利用できません。' },
        ],
      },
    ],
  },
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          { term: '素材が見つからない', description: '別PCや同期前のフォルダでプロジェクトを開いた場合は、右ペインでカットフォルダを登録し直し、素材カードの状態を確認します。' },
          ...
          { term: '書き出し前の確認', description: 'カット情報と尺、全ページ、修正シート、素材割当、CSPセル名、警告表示、紙シートを出力へ含める設定を一通り確認します。' },
        ],
      },
    ],
  },
