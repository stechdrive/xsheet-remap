...
it('edits sheet warp quadrilateral handles and applies template targets', async () => {
    ...
    fireEvent.pointerDown(sourceHandle, { pointerId: 1, button: 0, buttons: 1, clientX: 80, clientY: 80 })
    fireEvent.pointerMove(window, { pointerId: 1, buttons: 1, clientX: 90, clientY: 85 })
    fireEvent.pointerCancel(window, { pointerId: 1, buttons: 0, clientX: 90, clientY: 85 })
    expect(sheet.classList.contains('calibrationDragging')).toBe(false)
    expect(Number(document.querySelector('.calibrationHandle.source')?.getAttribute('cx'))).toBeCloseTo(expectedPoints[0].source.x)

    fireEvent.pointerDown(sourceHandle, { pointerId: 2, button: 0, buttons: 1, clientX: 80, clientY: 80 })
    fireEvent.pointerMove(window, { pointerId: 2, buttons: 1, clientX: 95, clientY: 90 })
    expect(sheet.classList.contains('calibrationDragging')).toBe(true)
    fireEvent.pointerUp(window, { pointerId: 2, buttons: 0, clientX: 100, clientY: 92 })
    expect(sheet.classList.contains('calibrationDragging')).toBe(false)
