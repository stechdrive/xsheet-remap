...
const templateHelpChapters: TemplateHelpChapter[] = [
  {
    ...
    steps: [
      ...
      { title: '「新しいテンプレート」を開く', description: '紙は名前、用紙サイズ、向き、PPI、任意の参照画像を設定します。デジタルは名前、FPS、初期フレーム数、CELLトラック数を設定します。' },
      { title: '作成条件を確認する', description: '紙は表示される実ピクセル数と参照画像のPPIを確認します。デジタルは実運用のFPSと必要なCELL列数を先に決め、「作成」を押します。' },
      ...
      { title: '必要な領域を追加する', description: '上部の「情報欄追加」「入力表を追加」「ACTION」「SOUND」「CELL」「CAMERA」「補助罫線」から必要なものだけを追加します。現状は個別の領域削除がないため、追加前に役割を確認します。' },
      ...
      { title: '表示ルールを整える', description: '「表示」で秒カウンター、紙の下端セル列名、ACTION・SOUND・CELL・CAMERAの見出し文字を確認します。空の見出しはシート上に表示されません。' },
    ...
    sections: [
      {
        title: '完成判定のチェックリスト',
        items: [
          { term: '見た目', description: 'テンプレート画像、アプリ描画の罫線、文字、カウンターが重ならず、全フレームをスクロールで確認できる。' },
    ...
    sections: [
      {
        ...
        items: [
          ...
          { term: 'デジタルタイムシート', description: 'FPS、初期フレーム数、CELLトラック数から連続キャンバスを作ります。後からトラックやレーンを増やすと横幅も追従します。' },
        ],
      },
      {
        title: '「新規テンプレ作成」メニュー',
  ...
  },
  {
    ...
    summary: 'タイミングを受ける4種類のグリッドと、紙・デジタルの列数の考え方を説明します。',
    sections: [
      {
        title: '4種類の役割',
        items: [
          { term: 'ACTION', description: '動作・原画系のタイミング欄です。偶数フレームのカウンターを右端に表示し、トラック構成に連動します。' },
          { term: 'SOUND', description: '台詞や音響指示の期間を受けるレーンです。列数は「領域一覧」から変更できます。' },
          { term: 'CELL', description: 'セル名とタイミングを入力する中心欄です。秒カウンターは左側に表示されます。デジタルではCELLトラック数に連動します。' },
          { term: 'CAMERA', description: '撮影指示の期間とレーンを受ける欄です。デジタルでもSOUNDと同様にレーン数に合わせて横方向へ増やせます。' },
        ],
      },
      {
        title: '行・列・開始フレーム',
        items: [
          ...
          { term: '列数', description: '各領域のトラック数またはレーン数です。紙は空間が限られるため収まる数にし、入り切らないものはEditor・Remap側で欄外扱いになります。' },
          { term: 'デジタルの可変幅', description: 'デジタルは予備列を確保せず、ACTION・CELL・SOUND・CAMERAの実数に合わせてキャンバスの横幅を広げます。トラック数の変更後は全体表示で配置を再確認します。' },
        ],
      },
    ],
  },
  {
    ...
    number: '07',
    title: '補助罫線と表示設定',
    summary: '入力を受けない罫線と、シート共通の見出し・カウンターを整えます。',
    sections: [
      {
        title: '補助罫線',
        items: [
      ...
      },
      {
        ...
        items: [
          ...
          { term: '秒カウンター', description: 'FPSに応じた秒数をCELL欄の左側に表示するか切り替えます。' },
    ...
    sections: [
      {
        ...
        items: [
          ...
          { term: 'デジタル固有', description: 'FPS、初期フレーム数、CELLトラック数を変更できます。フレーム数は各グリッドの行数へ、CELLトラック数はACTIONとCELLの列へ反映されます。' },
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          { term: '入力テスト', description: '情報欄と入力表のクリック範囲、ACTION・CELLのキー、SOUND・CAMERAの指示、メモ対象を確認します。' },
    ...
    sections: [
      {
        title: 'よくある状態',
        items: [
          { term: '領域を追加しすぎた', description: '現状のテンプレートEXEには、追加した領域を1件だけ削除する操作はありません。未適用なら「キャンセル」、適用済みなら保存済みJSONの読み直しまたは新規作成からやり直します。' },
