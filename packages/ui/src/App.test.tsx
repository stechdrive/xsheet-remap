...
it('renders the main workspace shell', () => {
  ...
  it('shows Editor-specific help instead of the Remap workflow', () => {
    ...
    expect(within(dialog).getByRole('heading', { name: 'セリフ音声を録音・編集してSOUNDへ割り付ける（任意）' })).toBeTruthy()
    expect(within(dialog).getByText(/Deleteは無音化、Shift\+Deleteはリップル削除/)).toBeTruthy()
    expect(within(dialog).getByText(/途中の間を含む1つのSOUND/)).toBeTruthy()
  ...
  it('opens the chapter-based Editor manual from help', () => {
    ...
    expect(within(dialog).getByText(/「検出しない」「発話区間を検出」「セリフ区間を自動作成」/)).toBeTruthy()
