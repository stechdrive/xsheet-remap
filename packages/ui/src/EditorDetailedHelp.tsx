    ...
    sections: [
      {
        title: 'セルへ入力する',
        items: [
          { term: '選択と文字入力', description: 'ACTIONには原画番号や中割り指示、CELLには動画番号を入力してEnterで確定します。タッチではセルを指で選び、表示される画面内入力で数字・空セル・中割・逆シート、確定とセル移動を操作できます。各セル列の最初のキーが1フレーム目より後ろに入力された場合は、先頭へカラセルを自動入力します。' },
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          { term: '録音後の処理', description: 'トラック見出しを右クリックし、「検出しない」「発話区間を検出」「セリフ区間を自動作成」から選びます。この設定は、その後に同じトラックへ録音・読み込みした音声へ適用されます。' },
          { term: '録音する', description: '上のルーラーをクリックして録音開始フレームへ再生ヘッドを置き、●を押します。録音中は■で終了し、結果は元録音を保持する非破壊クリップとして配置されます。' },
          { term: '音声ファイルを読み込む', description: '再生ヘッドと録音先トラックを決め、音声読込ボタンを押します。トラックの空白部分を右クリックして「再生ヘッドへ音声ファイルを読み込む」こともできます。音声がシート尺より長くても、読み込みだけではタイムシートのカット尺は変わりません。' },
        ],
      },
      {
        title: '再生・位置確認・表示',
      ...
      },
      {
        title: '選択と非破壊編集',
        items: [
          { term: '選択ツール（V）', description: 'クリップ、発話候補、セリフ区間をクリックして選びます。CtrlまたはShiftを押しながらクリックすると複数選択でき、クリップや区間はドラッグで移動できます。選択クリップは←／→でも1フレームずつ移動できます。空白からドラッグすると矩形選択になります。' },
          ...
          { term: '元に戻す・やり直す', description: '音声ツールバーの↶／↷で、音声クリップ、範囲編集、リンク区間の変更を戻したりやり直したりします。元音声は保持され、配置と参照範囲を編集する非破壊方式です。' },
        ],
      },
      {
        title: 'VAD候補をSOUNDへ割り付ける',
        items: [
          { term: 'VADとSOUNDの違い', description: '波形上の発話候補は音声を検出した根拠で、タイムシートのSOUND指示とは別データです。候補やセリフ区間を動かしても波形そのものは変わりません。' },
          { term: '検出設定と再検出', description: '歯車メニューで録音VAD環境を静か／通常／騒がしいから選び、検出感度と途切れにくさを調整します。トラックまたはクリップの右クリックメニューから再検出し、発話候補の右クリックメニューから無視／検出対象へ戻す操作ができます。状態表示が「Silero VAD」ならSileroモデルを使用中で、読み込めない場合だけ「簡易検出」と表示されます。' },
          { term: '新しいSOUNDを作る', description: '1つ以上の発話候補またはセリフ区間を選び、「音響指示へ割付…」を押します。CtrlまたはShiftで離れた候補を複数選ぶと、途中の間を含む1つのSOUNDとして話者・ラベルと本文を入力できます。時間範囲を選んで手動で割り付けることもできます。' },
          { term: '既存SOUNDへ追加する', description: '先にタイムシート側のSOUNDを選び、追加したい発話候補またはセリフ区間を選ぶと、ボタンが「選択中のSOUNDへ追加」に変わります。追加した区間は現在位置を保ったまま同じSOUNDへ関連付けられます。' },
        ],
      },
      {
        title: 'リンクの同期と保存',
        items: [
          ...
          { term: 'XSRへ保存される内容', description: '元音声はカット単位の共有資産、編集結果は資産を参照するクリップとして保存されます。VAD候補、セリフ区間、SOUNDへの割付はシート改訂ごとに保持され、同じ音声を分割してもXSR内へ同じ音声Blobを重複保存しません。' },
        ],
      },
    ],
  },
