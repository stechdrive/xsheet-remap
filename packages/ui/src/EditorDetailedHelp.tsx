    ...
    sections: [
      {
        ...
        items: [
          ...
          { term: '録音と音声編集', description: '録音対象のトラックを選び、再生ヘッドから録音します。録音中はほかのトラックを再生し、対象トラックへパンチインします。音声読込、ミュート／ソロ、再生ヘッド位置での無音1フレーム挿入・1フレーム削除も使えます。' },
          { term: 'セリフ区間を照合', description: '音声から検出した発話区間は緑枠で表示されます。検出感度を調整して再検出し、SOUND指示を選んで「選択SOUNDを検出区間へ」を押すと、最寄りの発話へ開始・終了を合わせられます。タイムライン上のSOUND指示も両端または中央をドラッグして直接調整できます。' },
          { term: '保存範囲', description: '3トラックの音声と検出結果はカット単位でXSRプロジェクトへ保存されます。SOUND指示はシート改訂側に残るため、音声を保ったまま改訂ごとの区間や表示内容を編集できます。' },
