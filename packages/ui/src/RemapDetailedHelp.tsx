    ...
    return {
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            { term: '1列だけコピー', description: '対象セル列の見出しを右クリックし、「AE用データをコピー」から日本語版AE／英語版AEを選びます。AEの表示言語に合わせると、カラセル用エフェクト名を正しく貼り付けられます。' },
            { term: '複数列をJSXで渡す', description: '書き出しメニューの「After Effects JSX」でACTIONまたはCELLを選びます。AEでスクリプトを実行し、列ごとに割り当て先レイヤーを選びます。素材FPSは割り当てたレイヤーから判定されるため、実際に使う素材レイヤーを選んでください。' },
            { term: '起動中のAEへ送る', description: 'Windows版ではAEを1プロセスだけ起動し、対象コンポジションをアクティブにしてから直接送信できます。送信後のAE画面で列とレイヤーを対応させ、既存のタイムリマップを置き換える確認内容を読んで確定します。' },
            { term: 'AE側の事前確認', description: '対象コンポジションの尺をシート以上にし、割り当てる素材レイヤーをロック解除して表示できる状態にします。素材がタイムリマップへ対応し、必要な長さを持つことも確認します。既存のタイムリマップやカラセル処理が見つかった場合は、置き換えてよい列だけ確定します。' },
