...
const remapHelpChapters: HelpChapter[] = editorHelpChapters.map(chapter => {
    ...
    return {
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            { term: '1列だけコピー', description: '対象セル列の見出しを右クリックし、「AE用データをコピー」から日本語版AE／英語版AEを選びます。AEの表示言語に合わせると、カラセル用エフェクト名を正しく貼り付けられます。通常の貼り付けでは、AE側で先にタイムリマップを有効にしてレイヤーを必要尺まで伸ばします。' },
            { term: '複数列をJSXで渡す', description: '書き出しメニューの「After Effects JSX」でACTIONまたはCELLを選びます。AEでスクリプトを実行し、列ごとに割り当て先レイヤーを選びます。素材FPSは割り当てたレイヤーから判定され、タイムリマップの有効化とシート尺までの延長も自動で行われます。' },
            { term: '起動中のAEへ送る', description: 'Windows版ではAEを1プロセスだけ起動し、対象コンポジションをアクティブにしてから直接送信できます。JSXと同じく、割り当てた素材レイヤーのタイムリマップ有効化と必要尺までの延長を自動で行います。' },
            { term: 'AE側の事前確認', description: '対象コンポジションの尺をシート以上にし、割り当てる素材レイヤーをロック解除します。素材内に必要なセル番号があり、タイムリマップへ対応することも確認します。既存のタイムリマップやカラセル処理が見つかった場合は、置き換えてよい列だけ確定します。' },
            { term: '特殊記号とカラセル', description: '中割・逆シートなどセル番号ではない記号は直前の値をHOLDします。カラセルは専用のブラインド効果で非表示にするため、変換前の確認事項が出た列は内容を見直します。' },
          ],
        },
      ],
    }
