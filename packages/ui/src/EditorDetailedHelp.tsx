...
const editorHelpChapters: HelpChapter[] = [
  ...
  },
  {
    ...
    number: '05',
    title: 'SOUND指示とセリフ音声',
    summary: '台詞の実音声を3トラックで録音・再生し、SOUND指示の開始・終了と照合します。',
    sections: [
      {
        title: '作成と編集',
        items: [
          { term: '区間から作成', description: 'SOUND欄をドラッグして開始から終了までの区間を選び、ダブルクリックまたはEnterで編集ダイアログを開きます。話者・ラベルと本文を入力し、Ctrl+Enterで確定できます。' },
          { term: '既存指示を編集', description: '表示されたSOUND指示をダブルクリックするか、選択してEnterで内容を編集します。端のハンドルで開始・終了を調整し、中央をドラッグして区間ごと移動します。' },
          { term: 'カットを通して再生', description: '下部の音声タイムラインで「カット頭から」または任意の再生ヘッド位置から再生します。3トラックを同時に鳴らすため、セリフ間の無音と掛け合いのリズムをカット全体で確認できます。選択SOUNDだけのループは必要なときだけオンにします。' },
          { term: '録音と範囲編集', description: '録音対象のトラックを選び、再生ヘッドから録音します。録音中はほかのトラックを再生し、対象トラックへパンチインします。トラックをドラッグして範囲を選ぶと、無音化、リップル削除、任意フレーム数の無音挿入、コピー／切り取り／上書き・挿入貼り付け、音声専用の元に戻す／やり直すを使えます。編集は元録音を保持する非破壊方式です。' },
          { term: '発話候補からSOUNDを作る', description: '検出された緑の発話候補をダブルクリックするか選択して「SOUNDを作成」を押すと、その区間が入った通常のSOUND入力画面が開きます。ラベルと内容を確定した時点でシートのSOUND欄へ新規作成されるため、先にSOUND指示を用意する必要はありません。無視した候補とリンク済み候補は状態を分けて表示します。' },
          { term: '再検出と同期', description: '検出感度を調整して再検出しても、リンク済み・無視済みの処理結果は保持します。無音挿入やリップル削除で後続音声を動かした場合はリンク済みSOUNDも同じフレーム数だけ移動します。リンク済み音声を無音化した場合や区間が一致しない場合はSOUNDを勝手に削除せず「要確認」として残します。' },
          { term: '保存範囲', description: '元音声はカット単位の共有資産、編集結果は資産を参照するクリップとしてXSRへ保存されます。同じ音声を分割してもXSR内ではハッシュBlobが重複しません。発話候補とSOUNDのリンクはシート改訂ごとに保持します。旧形式の音声プロジェクトは読込時に自動移行します。' },
    ...
    sections: [
      {
        ...
        items: [
          ...
          { term: 'タイミング調整', description: '指示の端をドラッグして区間を伸縮し、中央をドラッグして移動します。ピボットを持つ指示では変化の基準フレームも調整できます。' },
    ...
    sections: [
      {
        title: 'タイムラインメモ',
        items: [
          { term: 'メモを追加', description: 'シート上を右クリックして「メモを追加」を選びます。フレームや区間、選択したSOUND／CAMERA指示に結び付けて配置できます。' },
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          { term: '表示テンプレート', description: 'シート作業レールの「表示設定」で、プロジェクトに適用する表示様式を選びます。論理セル列とSOUND／CAMERA列は保持したまま、デジタルでは全列を表示して横幅を増やし、紙では用紙に収まる列だけを欄内へ投影します。' },
    ...
    sections: [
      {
        title: 'テンプレートを用意する',
        items: [
          { term: '新しいテンプレート', description: '紙タイムシートまたはデジタルタイムシートを選びます。紙用は用紙、向き、PPI、参照画像、デジタル用はFPS、初期フレーム数、ACTIONとCELLで共有するセル列数を設定します。' },
      ...
      },
      {
        title: 'レイアウトを編集する',
        items: [
          { term: '領域', description: '情報欄、入力表、ACTION、SOUND、CELL、CAMERA、入力を受けない補助罫線を追加し、キャンバス上のドラッグまたは数値で位置と大きさを調整します。不要な領域は「概要」または「領域一覧」から個別に削除できます。' },
          { term: '詳細設定', description: '領域の初期列数、行・列罫線、文字、情報欄の割当、メモ対象、補正基準枠、ページ寸法、見出し表示を編集します。論理セル列やSOUND／CAMERA列の実際の名前はシート入力ワークスペースで管理します。' },
