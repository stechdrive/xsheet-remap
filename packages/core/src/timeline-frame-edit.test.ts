...
import { sheetAnnotations, timelineMemos } from './sheet-memo'
...
describe('cut timeline frame editing', () => {
  ...
  it('clips deletion to the official cut and leaves page-bound content untouched', () => {
    ...
    const annotated = addAnnotation(source, annotation)
    const officialEnd = source.logicalSheet.frameOrigin + source.logicalSheet.durationFrames - 1

    const edited = applyCutTimelineFrameEdit(annotated, { kind: 'delete', frameStart: officialEnd + 1, frameCount: 20 })

    expect(edited).toBe(annotated)
    expect(sheetAnnotations(edited)).toEqual([annotation])
  })
  ...
  it('moves timeline memo anchors on insert without slicing their canvas or ink', () => {
    ...
    const withMemo = addTimelineMemo(source, memo)

    const edited = applyCutTimelineFrameEdit(withMemo, { kind: 'insert', atFrame: 6, frameCount: 3 })

    expect(timelineMemos(edited)[0]).toEqual({
      ...memo,
      texts: [],
      anchor: { ...memo.anchor, frame: 11 },
    })
  })

  it('deletes a memo only when its anchor frame is removed and shifts later anchors', () => {
    const source = timelineFixture()
    const makeMemo = (memoId: string, frame: number): TimelineInkMemo => ({
      kind: 'timeline',
      memoId,
    ...
    source.memos = [makeMemo('before', 4), makeMemo('anchored-inside', 7), makeMemo('after', 12)]
