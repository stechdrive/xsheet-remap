...
EXEでの保存は同じフォルダの一時ファイルへ全量を書き込み、ディスク同期後に置換します。2回目以降の保存では直前の正常ファイルを`<project>.xsr.bak`として1世代保持します。プロジェクト本体が破損している場合はバックアップから読み込みますが、バックアップ保護のため次回は保存先を指定します。旧`.xsr.json`は読み込みのみ対応し、次回保存時に`.xsr`へ移行します。
